...
knapsack_constraints[2, :] = np.abs(year2 - date_array) / budget3
...
def sproutpp(param_c: int, gnd: List[int], f_diff, ind_oracle, ind_add_oracle, num_sol: int = 0, k: int = 0,
    ...
    for gnd_combinatorial in gnd_combinatorials:
        ...
        budget3_new = budget3 - np.sum(np.abs(year2 - date_array)[list(gnd_combinatorial)])
        knapsack_constraints_new[2, :] = np.abs(year2 - date_array) / budget3_new
