...
tc_values = [int(n * 0.06)]  # ratio_tc_to_n = 0.06, tc = n * ratio_tc_to_n

for tc in tc_values:
    print(f"\nRunning SPROUT++ with tc = {tc}")
    ...
    def knapsack_feasible(sol: Set[int], knapsack_constraints: Union[np.ndarray, None]) -> bool:
        if knapsack_constraints is None:
            return True
        return np.all(np.sum(knapsack_constraints[:, list(sol)], axis=1) <= 1)

    sol, f_val, _ = sproutpp(param_c, gnd, f_diff, ind_oracle, ind_add_oracle, knapsack_constraints=knapsack_constraints, extendible=True ,num_sol=num_sol, epsilon=epsilon, k=1, param_alpha=0.5)
    print(f"SPROUT++ Solution (tc={tc}):", sol, "Cut Value:", f_val)