...
def repeated_greedy_alg(pq: SortedDict, num_sol: int, f_diff, ind_add_oracle,
                        knapsack_constraints: Union[np.ndarray, None], density_ratio: float,
                        epsilon: float, opt_size_ub: int, verbose: bool = False) -> Tuple[Set[int], float, int, int, bool]:
    best_sol = None
    ...
    for i in range(num_sol):
        sol, f_val, num_f, num_or, greedy_kr = greedy(
            SortedDict(pq), f_diff, ind_add_oracle, knapsack_constraints=knapsack_constraints,
            density_ratio=density_ratio, epsilon=epsilon, opt_size_ub=opt_size_ub)
        ...
        keys_to_remove = [k for k in pq.values() if k[0] in sol]
...
def repeated_greedy_density_search(pq: SortedDict, num_sol: int, f_diff, ind_add_oracle,
                                   knapsack_constraints: Union[np.ndarray, None], beta_scaling: float,
                                   delta: float, epsilon: float, opt_size_ub: int, verbose: bool = False) -> Tuple[Set[int], float, int, int]:
    ...
    while upper_density_ratio > (1 + delta) * lower_density_ratio:
        ...
        sol, f_val, num_f, num_or, knap_reject = repeated_greedy_alg(
            SortedDict(pq), num_sol, f_diff, ind_add_oracle, knapsack_constraints, density_ratio, epsilon, opt_size_ub)
...
def init_rg_params(num_sol: int, k: int, monotone: bool, knapsack_constraints: Union[np.ndarray, None], epsilon: float) -> Tuple[int, bool, float]:
    ...
    if ignorable_knapsack(knapsack_constraints):
        ...
        assert k > 0, "k must be specified when knapsack constraints are provided"
    ...
    if monotone:
        ...
        if num_sol == 0:
            num_sol = int(np.floor(1 + np.sqrt(2*(k + 2*m + 1) / usm_a)))
        beta_scaling = 2 * (1 - epsilon) * (1 - 1/num_sol - epsilon) / (k + 2*m + 1 + usm_a*(num_sol - 1)/2)

    return num_sol, run_density_search, beta_scaling

def repeated_greedy(gnd: List[int], f_diff, ind_add_oracle, num_sol: int = 0, k: int = 0,
                    knapsack_constraints: Union[np.ndarray, None] = None, monotone: bool = False,
                    epsilon: float = 0.0, opt_size_ub: int = None, verbose_lvl: int = 1) -> Tuple[Set[int], float, int, int]:
    ...
    if run_density_search:
        delta = epsilon
        best_sol, best_f_val, num_f, num_or = repeated_greedy_density_search(
            pq, num_sol, f_diff, ind_add_oracle, knapsack_constraints, beta_scaling, delta, 0.0, opt_size_ub, verbose=alg_verbose)
    else:
        density_ratio = 0.0
        best_sol, best_f_val, num_f, num_or = repeated_greedy_alg(
            pq, num_sol, f_diff, ind_add_oracle, knapsack_constraints, density_ratio, epsilon, opt_size_ub, verbose=alg_verbose)
