...
def greedy_with_pq(pq: List, f_diff, ind_add_oracle, knapsack_constraints: Union[np.ndarray, None] = None, density_ratio: float = 0.0, epsilon: float = 0.0, opt_size_ub: int = None, verbose: bool = False) -> Tuple[Set[int], float, int, int, bool]:
...
def greedy(gnd: List[int], f_diff, ind_add_oracle, knapsack_constraints: Union[np.ndarray, None] = None, density_ratio: float = 0.0, epsilon: float = 0.0, opt_size_ub: int = None, verbose: bool = False) -> Tuple[Set[int], float, int, int, bool]:
    if opt_size_ub is None:
        opt_size_ub = len(gnd)
